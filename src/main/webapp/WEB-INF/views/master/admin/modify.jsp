		...
		<div class="row">
			<div class="col-12">
				<div class="card card-info">
					...
					<!-- form start -->
					<form role="form" id="quickForm" method="post" action="modify-proc.do" onsubmit="return validation();">
						...
						<div class="card-body">
							...
							<div class="form-group row">
								...
								<div class="col-sm-3">
									<input type="text" name="id" class="form-control" id="idno" value='<c:out value="${data.id }"></c:out>' placeholder="사번" readonly>
								</div>
							</div>
							<div class="form-group row">
								<label for="inputPassword3" class="col-sm-2 col-form-label">비밀번호</label>
								<div class="col-sm-4">
									<input type="password" name="password" class="form-control" id="inputPassword3" placeholder="비밀번호" >
								</div>
							</div>
							<div class="form-group row">
								<label for="name" class="col-sm-2 col-form-label">이름</label>
								<div class="col-sm-3">
									<input type="text" name="name" class="form-control" id="name" value='<c:out value="${data.name }"></c:out>' placeholder="이름">
								</div>
							</div>
							<div class="form-group row">
								<label for="dept" class="col-sm-2 col-form-label">부서</label>
								<div class="col-sm-3">
									<input type="text" name="dept" class="form-control" id="dept" value='<c:out value="${data.dept }"></c:out>' placeholder="부서">
								</div>
							</div>
							<div class="form-group row">
								<label for="email" class="col-sm-2 col-form-label">Email</label>
								<div class="col-sm-3">
									<input type="text" name="email" class="form-control" id="email" value='<c:out value="${data.email }"></c:out>'  placeholder="email">
								</div>
							</div>
							<div class="form-group row">
									...
									<div class="row">
										<div class="col-sm-2">
											<div class="icheck-primary d-inline">
												<input type="radio" id="auth-master" name="auth" value="master" ${data.auth == 'master' ? 'checked' : '' }>
												<label for="auth-master">MASTER</label>
											</div>
										</div>
										<div class="col-sm-2">
											<div class="icheck-primary d-inline">
												<input type="radio" id="auth-general" name="auth" value="general" ${data.auth == 'general' ? 'checked' : '' }>
...
<script>
	$(function() {
		<c:forEach items="${data.authList}" var="item">
			$("input[name=role][value=${item}]").prop("checked", true);
		</c:forEach>
	});
